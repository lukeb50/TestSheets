...
const newResponseMenu = document.getElementById("newResponseMenu");

function showMatchingScreen(sheetContainer) {
    dataContainer = sheetContainer;
    sheetInterpreter = new SheetInterpreter(sheetContainer, fieldData);
    selectedSheet = getSheetFromIdentifier(sheetContainer.sheetId);
    miscData = {};
    ...
    miscData['filteredResponses'] = dataContainer.getResponseIds();
    ...
    dataContainer.includedQuestions.forEach((questionId) => {
        if (sheetInterpreter.getExcludedFields().indexOf(sheetInterpreter.getFieldNameFromQuestionId(questionId)) === -1) {
            ...
            let matchResult = sheetInterpreter.getFieldNameFromQuestionId(questionId);
            if (matchResult) {
    ...
    newColumnButton.onclick = function () {
        ...
        for (const response of dataContainer.getResponses()) {
            ...
            answerInput.value = response.getAnswer(newQuestionId).getContent();
        }
    };
    //Create main table
    var currentCount = 1;
    ...
    manualButton.onclick = async function (clickEv) {
        clickEv.stopPropagation();
        showNewReponseMenu(lastActiveSource && lastActiveSource.getIsRefreshable()).then((action) => {
            processNewResponseAction(action);
        }).catch(() => {
            //Drop silently
        }).finally(() => {
            newResponseMenu.style.display = "none";
        })
    };
    return new Promise((resolve, reject) => {
        ...
        matchingConfirmBtn.onclick = function () {
            showDialog(courseInfoDialog);
            forceSave();//non-await as user is staying on page
            let containerResponseKeys = dataContainer.getResponseIds();
            miscData['selectedResponses'].sort((a, b) => {
                return containerResponseKeys.indexOf(a) - containerResponseKeys.indexOf(b);
            });
            let reducedFinalSheet = new MergeDivideSheetTransformer(dataContainer, selectedSheet, fieldData).execute();
            new FieldModificationSheetTransformer(reducedFinalSheet, selectedSheet, fieldData).execute(FieldModificationSheetTransformer.MODIFICATION_STAGE.GENERATION);
            getCourseInformation().then((courseInfo) => {
                ...
                new TestSheetPdfGenerator().generatePdfFile(selectedSheet, reducedFinalSheet, miscData['selectedResponses'], courseInfo).then((files) => {
    ...
    async function processNewResponseAction(action) {
        switch (action) {
            case "blank":
            ...
            case "currentSource":
                showSpinner();
                try {
                    let newRaw = await lastActiveSource.execute();
                    mergeRawIntoSheet(newRaw);
                } catch (err) {
                    alert("Unable to add new responses");
                    console.log(err);
                } finally {
                    hideSpinner();
                }
            ...
            case "differentSource":
                ...
                sourceSelector.contentWindow.getDataFromSource(selectedSheet, [SOURCE_CATEGORIES.ONLINE, SOURCE_CATEGORIES.FILE]).then(({ rawData, source }) => {
                    lastActiveSource = source;
                    mergeRawIntoSheet(rawData);
                }).catch((err) => {
                    console.log(err);
                }).finally(() => {
                    hideDialog();
        ...
        function mergeRawIntoSheet(rawData) {
            //Perform matching on raw
            let matchingEngine = new SourceMatchingEngine(rawData, selectedSheet.identifier, fieldData);
            let rawMatchingData = matchingEngine.performMatching();
            let matchingData = dataContainer.matching;
            //Find any columns that can be lined up and aren't already
            Object.keys(matchingData).forEach((matchingFieldName) => {
                if (Object.keys(rawMatchingData).includes(matchingFieldName) && rawMatchingData[matchingFieldName] !== matchingData[matchingFieldName]) {
                    //They both have the same field matched, but the questionIds are different.
                    //Adjust the question Id on the raw data.
                    rawData.getResponses().forEach((response) => {
                        response.getAnswer(rawMatchingData[matchingFieldName]).questionId = matchingData[matchingFieldName];
                    })
                }
            })
            //Transform data
            new FieldModificationSheetTransformer(new SheetTransformerSheetOutput(rawData.getResponses(), dataContainer.matching), selectedSheet, fieldData)
                .execute(FieldModificationSheetTransformer.MODIFICATION_STAGE.LOAD);
            //Load into main sheet
            rawData.getResponses().forEach((rawResponse) => {
                dataContainer.addResponse(Response.fromResponse(rawResponse, Answer));
            })
            //Trigger a full re-render, it's not worth calculating and manually rendering each added row/column.
            showMatchingScreen(dataContainer);
        }
    }
}

newResponseBlankButton = document.getElementById("blankSourceButton");
newResponseCurrentButton = document.getElementById("currentSourceButton");
newResponseDifferentButton = document.getElementById("differentSourceButton");

function showNewReponseMenu(showCurrentSource = false) {
    let scrollListener;
    let clickListener;
    //Show
    newResponseMenu.style.display = "block";
    newResponseCurrentButton.style.display = showCurrentSource ? "flex" : "none";
    //Calculate menu positioning
    let menuHeight = newResponseMenu.getBoundingClientRect().height;
    let manualButton = document.getElementById("mainSelectionTableManualResponseBtn");
    buttonRect = manualButton.getBoundingClientRect();
    let screenBottom = matchingTableHolder.getBoundingClientRect().bottom;
    if (buttonRect.bottom + 10 + menuHeight < screenBottom) {
        //Show below button
        newResponseMenu.style.top = `${buttonRect.bottom + 5}px`
    } else {
        //Show above button
        newResponseMenu.style.top = `${buttonRect.top - menuHeight - 5}px`
    }
    return new Promise((resolve, reject) => {
        scrollListener = matchingTableHolder.addEventListener("scroll", rejectTeardown, { once: true });
        clickListener = document.body.addEventListener("click", rejectTeardown, { once: true });

        newResponseMenu.addEventListener("click", (e) => {
            e.stopPropagation();
            let button = e.target.closest("button");
            if (button) {
                resolveTeardown(button.getAttribute("data-action"));
            }
        })

        function rejectTeardown() {
            teardown();
            reject();
        }

        function resolveTeardown(data) {
            teardown();
            resolve(data);
        }

        function teardown() {
            matchingTableHolder.removeEventListener("scroll", scrollListener);
...
function createRow(responseId, response, currentCount) {
    ...
    dataContainer.includedQuestions.forEach((questionId) => {
        ...
        if (sheetInterpreter.getExcludedFields().indexOf(sheetInterpreter.getFieldNameFromQuestionId(questionId)) === -1) {
            ...
            answerInput.onchange = handleInputUpdate;
            answerInput.value = answerObj.getContent();
        }
    });
...
function handleInputUpdate(e) {
    ...
    dataContainer.getResponse(responseId).getAnswer(questionId).setContent(target.value);
    markChange();
    handleAutofillButtons();
    //Re-run each verification module if affected
    //Determine the name of the field being modified
    var selectedFieldMatching = sheetInterpreter.getFieldNameFromQuestionId(questionId);
...
function fillSelector(selector, questionId) {
    fields = sheetInterpreter.getFullFields();
...
function handleAutofillButtons() {
    ...
    sheetInterpreter.getFullFields().forEach((field) => {
            ...
            fieldData[field].Autofill.forEach((autofillCombination) => {
                ...
                if (autofillCombination.every(val => selectedFields.includes(val))) {
                    ...
                    for (const response of dataContainer.getResponses()) {
                        ...
                        if (!answer || answer.getContent().length === 0 || answer.getContent() === "") {
                            ...
                            autofillCombination.forEach((combinationField) => {
                                ...
                                let fieldContents = response.getAnswer(fieldId).getContent();
                ...
                if (autofills[fieldName][a].RequiredFields.length === autofillOptionToUse.length) {
                    ...
                    if (autofills[fieldName][a].RequiredFields.every(val => autofillOptionToUse.includes(val))) {
                        ...
                        autofills[fieldName][a].function(dataContainer, responseId).then((autofilledValue) => {
                            ...
                            dataContainer.getResponse(responseId).getAnswer(questionId).getContent() = autofilledValue
                            markChange();
                            button.remove();
                        }).catch((err) => {
...
function applyFilters() {
    ...
    if (filteredFields.length > 0) {
        for (const response of dataContainer.getResponses()) {
            ...
            filteredFields.forEach((fieldName) => {
                ...
                let answerContent = response.getAnswer(fieldName).getContent();
    ...
    //Update visibilities
    dataContainer.getResponseIds().forEach((responseId) => {
