...
window.onload = async function () {
    ...
    resolveAllPromises({
        dataFiles: loadJsonFiles(dataFileNames.sheets, dataFileNames.toolkit),
        ...
        sheetContainer = loadResults.sheetInfoBuilder.build();
...
function createIndividualContainer(individualEntry, sheetToolkitData, flagConfigs) {
    ...
    if (nameQuestionId) {
        ...
        for (response of sheetContainer.getResponses()) {
            let responseId = response.responseId;
            let opt = createElement("option", candidateNameSelect, response.getAnswer(nameQuestionId).getContent());
    ...
    function renderResultsList() {
        ...
        for (let i = 0; i < cardCount; i++) {
            ...
            switch (queryType) {
                case "candidate":
                    if (!sheetContainer.matching['Name']) {
                        ...
                        let allResponseIds = sheetContainer.getResponseIds();
                        queryValue = allResponseIds[i];
                        card.querySelector("h1.resultCardLoadTitle").textContent = getCandidateName(queryValue);
                    }
                    break;
                case "skill":
                    let interpreter = new ToolkitMappingFullInterpreter(sheetContainer)
            ...
            loadButton.addEventListener("click", async () => {
                ...
                switch (queryType) {
                    case "candidate":
                        card.querySelector(".resultTitle").textContent = getCandidateName(rawValue);
                ...
                pdfGenerateButton.addEventListener("click", () => {
                    ...
                    switch (queryType) {
                        case "candidate":
                            reportTitle = getCandidateName(rawValue);
                            let includedSkills = findIncludedCandidateSkills(queryRelatedIds);
                            includedSkills.forEach((skillId) => {
                                let skillName = toolkitData.sheets[sheetContainer.getSheetIdentifier()]?.skills[skillId]?.name ?? "Skill Name Unavailable";
                                //queryRelatedIds span multiple skills, pick only the Ids that correspond to this skill
